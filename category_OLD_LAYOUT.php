...
if (!is_numeric($album_id) || !PicasaWebAlbum::isAlbum(PICASA_USER, $album_id)) {
  header('Location: ../..');
  die();
}
...
// make sure name in URL matches name of album with id $album_id
if ($album_name != PicasaWebUser::makeLink($album->getTitle())) {
  $url = '/category/' . $album_id . '/' . PicasaWebUser::makeLink($album->getTitle());
  header('Location: ' . $url);
  die();
}
